            ...
            <div class="section-detail">
                ...
                <div class="table-responsive">
                    <table class="table list-table-wp">
                        ...
                        <tbody>
                            @foreach ($list_account_user as $key => $account)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td class="clearfix">
                                        <div class="tb-title fl-left">
                                            <a href="" title="">{{ $account->customer_name }}</a>
                                        </div>
                                        <ul class="list-operation fl-right">
                                            <li> <a onclick="return confirm('Bạn có muốn xoá {{ $account->customer_name }}?')"
                                    ...
                                    <td><span class="tbody-text">{{ $account->customer_email }}</span></td>
                                    <td><span class="tbody-text">{{ $account->customer_phone }}</span></td>
                                    <td><span class="tbody-text">{{ $account->customer_address }}</span></td>
                ...
                @if (isset($_GET['key']))
                    {{ $list_account_user->appends(['key' => $_GET['key']])->links('/vendor/pagination/bootstrap-4') }}
                @elseif(isset($_GET['actions']))
                    {{ $list_account_user->appends(['actions' => $_GET['actions']])->links('/vendor/pagination/bootstrap-4') }}
                @else
                    {{ $list_account_user->links('/vendor/pagination/bootstrap-4') }}
                @endif
            </div>
        </div>
    </div>
